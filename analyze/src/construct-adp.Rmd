...
Constructing ADP "capacity" proxy measure for all sampled facilities.

Data source: ICE facilities report `ICE_Facility_List_11-06-2017-web.xlsx` released by NIJC.

Next steps:
- National level comparisons of approaches
- Test for monthly ADP observations
- Compare which "baseline" highest per facility/AOR
- Combined approaches?
- Statistical testing?
...
Comparing first two capacity proxy approaches for all sampled facilities. Trends are parallel because we are deciding where to set "capacity" baseline versus same sets of yearly ADP observations. Note variation: "versus max ADP ever" approach is usually a higher baseline, which makes sense, but sometimes it's lower or approaches very similar. What would this look like at national level?

```{r compare_approaches, echo=FALSE, message=FALSE, warning=FALSE}

# There's probably a more streamlined way to do this (ignoring duplicate fields in approach2)
...
Comparing first two capacity proxies with reported Nov. 2017 capacity where known, excluding "AS NEEDED" facilities. Known capacity tends to be higher baseline than either capacity proxy, farthest off in HOU, ELP, PHO.

```{r compare_approaches_known_capacity, echo=FALSE, message=FALSE, warning=FALSE}
...
compare_capacity <- function(x) (data$capacity - x)

data <- data %>% 
  mutate(across(c(ends_with("adp"), -contains("compare"), -contains("max")),
                compare_capacity,
                .names = "compare_capacity_{col}"))

approach3_data <- data %>%
  group_by(aor) %>% 
  summarize_at(vars(contains("compare_capacity")), sum) %>%
  pivot_longer(
    cols = starts_with("compare_capacity"),
    names_to = "fy",
    values_to = "versus_capacity",
    values_drop_na = TRUE
  )

approach3_data$fy <- approach3_data$fy %>% str_replace_all('[aA-zZ]', '')
approach3_data$fy <- paste('20', approach3_data$fy, sep='')
approach3_data$fy <- as.integer(approach3_data$fy)

# Refreshing previous approaches filtering out "AS NEEDED"
...
to_plot <- merge(approach1_data, approach2_data, by=c('aor', 'fy'))
to_plot <- merge(to_plot, approach3_data, by=c('aor', 'fy'))8_

to_plot <- to_plot %>% 
  pivot_longer(
    cols = c("versus_capacity", "max_versus_fy18", "max_versus_adp_ever"),
    names_to = "approach",
    values_to = "capacity",
    values_drop_na = TRUE
  )
...
```{r compare_baselines, echo=FALSE, message=FALSE, warning=FALSE}

print(data[c('detloc', 'aor', 'capacity', 'fy18_max_population_count', 'max_adp')])
