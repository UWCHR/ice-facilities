...
# pivot adp wide to long
df_aor_adp_long <- df_aor_adp %>% 
  pivot_longer(
    cols = starts_with("fy"),
    names_to = "key",
    values_to = "cases",
...
# convert fiscal_year to factor
df_aor_adp_long <- df_aor_adp_long %>%
  mutate(fiscal_year = as.factor(fiscal_year))
class(df_aor_adp_long$fiscal_year) 

# recode fiscal_year values
levels(df_aor_adp_long$fiscal_year)[levels(df_aor_adp_long$fiscal_year)=='fy18'] <- '2018'
levels(df_aor_adp_long$fiscal_year)[levels(df_aor_adp_long$fiscal_year)=='fy17'] <- '2017'
levels(df_aor_adp_long$fiscal_year)[levels(df_aor_adp_long$fiscal_year)=='fy16'] <- '2016'
levels(df_aor_adp_long$fiscal_year)[levels(df_aor_adp_long$fiscal_year)=='fy15'] <- '2015'
levels(df_aor_adp_long$fiscal_year)[levels(df_aor_adp_long$fiscal_year)=='fy14'] <- '2014'
levels(df_aor_adp_long$fiscal_year)[levels(df_aor_adp_long$fiscal_year)=='fy13'] <- '2013'
levels(df_aor_adp_long$fiscal_year)[levels(df_aor_adp_long$fiscal_year)=='fy12'] <- '2012'
levels(df_aor_adp_long$fiscal_year)[levels(df_aor_adp_long$fiscal_year)=='fy11'] <- '2011'
levels(df_aor_adp_long$fiscal_year)[levels(df_aor_adp_long$fiscal_year)=='fy10'] <- '2010'
levels(df_aor_adp_long$fiscal_year)[levels(df_aor_adp_long$fiscal_year)=='fy09'] <- '2009'

# plot adp levels
  ggplot(df_aor_adp_long) + 
    geom_histogram(mapping = aes(x = adp))

# plot adp by fiscal year across aor
ggplot(data = df_aor_adp_long,
...
# plot adp by fiscal year across aor with facet wrap 
ggplot(data = df_aor_adp_long,
       aes(x = fiscal_year, y = adp, group = aor, color = aor)) + 
  geom_line() +
  xlab("Fiscal Year") +
 ...
 ggtitle("Average Daily Population (ADP), 2009-2018") +
  ...
  facet_wrap(~ aor) + 
  theme(axis.text.x = element_text(angle = 90, size = 8, vjust = .5))
ggsave("adp_facets.pdf")

# # plot
# ggplot(data = df_aor_adp_long, aes(x = fiscal_year, y = adp, group = aor)) +
#   geom_line() +
#   geom_line(stat = "smooth", method = "loess",
#             aes(group = aor)) +
#   facet_wrap(~ aor) +
...
# encounters
  ...
  # visualize distributions for encounters
    ...
    # plot encounters by fiscal year across aor facet wrap 
    ggplot(data = df_aor_adp_enforcement_long_complete,
           aes(x = year, y = n_encounters, group = aor, color = aor)) + 
      geom_line() +
      xlab("Fiscal Year") +
...
# visualize distributions for arrests
  ...
  # visualize distributions for arrests
    ...
    # plot arrests by fiscal year across aor facet wrap 
    ggplot(data = df_aor_adp_enforcement_long_complete,
           aes(x = year, y = n_arrests, group = aor, color = aor)) + 
      geom_line() +
      xlab("Fiscal Year") +
...
# removals
  ...
  # visualize distributions for removals
    ...
    # plot removals by fiscal year across aor facet wrap 
    ggplot(data = df_aor_adp_enforcement_long_complete,
           aes(x = year, y = n_removals, group = aor, color = aor)) + 
      geom_line() +
      xlab("Fiscal Year") +
...
```{r estimate_correlation_adp_arrests, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
The visualization of choice when exploring the relationship between two variables, such as average daily population and arrests, is a scatterplot. Let us first create a scatterplot that illustrates the relationship between average daily population and the number of arrests.
...
```{r scatterplot_arrests, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
plot_log_adp_arrests <- ggplot(df_aor_adp_enforcement_long_complete, aes(x=log(adp), y=log(n_encounters))) + 
  geom_point(alpha = 0.5)  
...
#### Visualize bivariate time series
How does ADP and number of arrests vary across time?

```{r visualize_bivariate_time_series, eval=FALSE}
#https://clanfear.github.io/CSSS508/Lectures/Week2/CSSS508_Week2_ggplot2.html#98

ggplot(df_aor_adp_enforcement_long_complete, aes(x = fiscal_year, y = ..count.., group = aor)) + 
  geom_line(aes(y = n_arrests, color = "n_arrests")) + 
  geom_line(aes(y = adp, color = "adp")) +
  theme_bw() + 
  facet_wrap(~ aor) 
```
...
model1 <- lm(log(n_arrests) ~ log(adp), data = df_aor_adp_arrests_long)
...
ggplot(df_aor_adp_arrests_long) +
  geom_density(aes(n_arrests))

model_log <- lm(log(n_arrests) ~ log(adp), data = df_aor_adp_arrests_long)
...
ggplot(df_aor_adp_arrests_long, aes(adp, n_arrests)) + 
  geom_hex(bins = 50)

newdf <- df_aor_adp_arrests_long %>% 
