...
df_aor_adp_pivot$fy <- df_aor_adp_pivot$fy %>% str_replace_all('[aA-zZ]', '')

df_aor_adp_pivot$fy <- paste('20', df_aor_adp_pivot$fy, sep='')

ggplot(data = df_aor_adp_pivot,
       aes(x = fy, y = adp, group = aor, color = aor)) + 
  geom_line() +
  xlab("Fiscal Year") +
  ylab("ADP") +
  theme_bw() + 
  facet_wrap(~ aor) + 
  theme(axis.text.x = element_text(angle = 90, size = 8, vjust = .5)) +
  ggtitle("AOR Total ADP per Fiscal Year")
