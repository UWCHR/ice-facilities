...
# Preliminary analysis of population-weighted detention & enforcement metrics

Note this is using a single 5-year ACS population estimate for each AOR in 2018, constructed from mapping of counties to AORs in `county_aor.R`.
...
county_aor <- read_delim(
  here::here('analyze', 'hand', 'county_aor.csv'), 
  delim = ",")
...
Scatter plot of ADP per capita by arrests per capita; each observation is AOR/FY total. We see two clear groups of AORs: Southern border AORs have higher ADP and arrests per capita.
...
Same plot as first above, but not logged, just as visual proof that log transformation does not change relationships between observations.
...
Slightly different apparent relationship between ecncounters and ADP: note high rate of encounters per capita in NYC.
...
Very strong relationship between removals and ADP per capita; high ADP facilities on southern U.S. border likely used as staging for deportations.
...
Notable disjunction in trend of removals versus arrests per capita. Southern border AORs have higher rates of both but wide spread; close but weakly positive (?) relationship for other AORs.
...
Again note high rate of encounters in NYC, LOS versus rate of arrests. Suggestive of dragnet-style operations with high community impact, low enforcement outcomes?
...
Strong, significant positive relationship between arrests per capita, ADP per capita. 
...
Weaker, significant relationship between arrests per capita, ADP per capita.
...
Significant relationship between encounters, ADP per capita.
...
Combining multiple independent variables: significant relationship between removals, ADP; arrests not signficant.

```{r removals_arrests_adp_lm}

model1 <- lm(removals_per_capita ~ adp_per_capita + arrests_per_capita, data = aor_enforce_adp)
...
Other direction same result: removals signficant, arrests not.

```{r adp_removals_arrests_lm}

model1 <- lm(adp_per_capita ~ removals_per_capita + arrests_per_capita, data = aor_enforce_adp)
...
Both arrests significant relationship with ADP, but encounters not significant:

```{r adp_encounters_arrests_lm}

model1 <- lm(adp_per_capita ~ encounters_per_capita + arrests_per_capita, data = aor_enforce_adp)
